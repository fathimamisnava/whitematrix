...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('https://whitematrix-v1dl.onrender.com/api/users/login', {
                    method: 'POST',
    ...
    if (registerForm) {
        registerForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('https://whitematrix-v1dl.onrender.com/api/users/register', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    if (usernameElement) {
        ...
        fetch('https://whitematrix-v1dl.onrender.com/api/users/me', {
    ...
    if (grievanceForm) {
        grievanceForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('https://whitematrix-v1dl.onrender.com/api/grievances/submit', {
                    method: 'POST',
                    headers: { 'Authorization': `Bearer ${token}` },
                    body: formData
                });
    ...
    if (grievancesContainer) {
        ...
        fetch('https://whitematrix-v1dl.onrender.com/api/grievances/my-grievances', {
            headers: { 'Authorization': `Bearer ${token}` }
        })
        .then(response => {
        ...
        .then(grievances => {
            grievancesContainer.innerHTML = grievances.map(grievance => `
                <div class="item">
                    ...
                    ${grievance.supportingDoc ? `<a href="https://whitematrix-v1dl.onrender.com/api/grievances/file/${grievance._id}" target="_blank">View Document</a>` : '<p>No supporting document</p>'}
                </div>
            `).join('');
        })
        .catch(error => {
    ...
    if (adminGrievancesContainer) {
        ...
        fetch('https://whitematrix-v1dl.onrender.com/api/grievances/admin/all', {
            headers: { 'Authorization': `Bearer ${token}` }
        })
        .then(response => {
            if (!response.ok) throw new Error('Error fetching grievances');
